...
function TwitchEmotes_SlightlyUnstable_RenderSuggestion(text)
    local fullEmotePath = TwitchEmotes_SlightlyUnstable_Emoticons_Pack[text];
...
function TwitchEmotes_SlightlyUnstable:SetAutoComplete(value)
    if value and not autocompleteInited then
        local i = tablelength(AllTwitchEmoteNames);
        for k, _ in pairs(TwitchEmotes_SlightlyUnstable_Emoticons_Pack) do
        ...
        for i=1, NUM_CHAT_WINDOWS do
            ...
            local autocompletesettings = {
                ...
                renderSuggestionFN = TwitchEmotes_SlightlyUnstable_RenderSuggestion,
